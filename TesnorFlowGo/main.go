...
func main() {
	...
	if len(os.Args) < 2 {
		log.Fatal("usage: imgrecognition <image_url>")
	}
	fmt.Printf("url: $s\n" os.Args[1])
	...
	if err != nil {
		log.Fatalf("unable to make tensor from image: %v", err)
	}
...
func loadMoel() (*tensorflow.Graph, []string, error) {
...
func normalizeImage(body io.ReadCloser) (*tensorflow.Tensor, error) {
	...
	session, err := session.NewSession(graph, nil)
	if err != nil {
		return nil, err
	}
...
func getNormalizedGraph() (graph *tensorflow.Graph, input, output tensorflow.Output, err error) {
	...
	output = op.Sub(s,
		...
		op.Cons(s.SubScope("mean"), float32(117)))
